...
# ==========================
# Pfad-Konfiguration (portabel)
# ==========================

# Basis: Ordner, in dem diese Datei liegt
BASE_DIR = Path(__file__).resolve().parent


def resolve_paths():
    """
    Ermittelt SOURCE_DIR und DASHBOARD_DIR ohne fest verdrahtete Laufwerke.

    Priorität:
    1. Kommandozeilenargumente:
       python build_dashboard.py <source_dir> <dashboard_dir>

    2. Optional: dashboard_config.json im selben Ordner wie dieses Script:
       {
         "source_dir": "C:/irgendwo/aktien",
         "dashboard_dir": "C:/irgendwo/dashboard_aktien"
       }

    3. Fallback:
       - SOURCE_DIR    = BASE_DIR / "aktien_roh"
       - DASHBOARD_DIR = BASE_DIR
    """
    # 1) CLI-Argumente
    args = sys.argv[1:]
    source_dir_arg = args[0] if len(args) >= 1 else None
    dashboard_dir_arg = args[1] if len(args) >= 2 else None

    # 2) Config-Datei (optional)
    ...
    if config_file.exists():
        try:
            ...
            print(f"[WARN] Konnte dashboard_config.json nicht lesen: {e}", file=sys.stderr)

    # 3) Auflösen mit Priorität
    if source_dir_arg:
        source_dir = Path(source_dir_arg)
    else:
        source_dir = Path(config.get("source_dir", BASE_DIR / "aktien_roh"))

    if dashboard_dir_arg:
        dashboard_dir = Path(dashboard_dir_arg)
    else:
        dashboard_dir = Path(config.get("dashboard_dir", BASE_DIR))

    return source_dir, dashboard_dir


# Aufgelöste Pfade
SOURCE_DIR, DASHBOARD_DIR = resolve_paths()
...
def load_json_files(source_dir: Path):
    ...
    if not source_dir.exists():
        print(f"[WARN] SOURCE_DIR existiert nicht: {source_dir}", file=sys.stderr)
        return records
...
def main():
    ...
    print(f"[INFO] DASHBOARD_DIR    : {DASHBOARD_DIR}")
    print(f"[INFO] TARGET_JSON_DIR  : {TARGET_JSON_DIR}")
    print(f"[INFO] OUTPUT_JS_FILE   : {OUTPUT_JS_FILE}")
